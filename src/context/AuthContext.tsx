...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const register = async (
    ...
    try {
      ...
      toast.success(`Account created successfully! Your 2FA code is: ${otp}`);

      // Do not set user yet, wait for OTP verification
      // setUser(newUser);
  ...
  const verifyOTP = async (email: string, otp: string): Promise<void> => {
    ...
    try {
      ...
      // In a real app, this would verify the OTP against the stored one

      // OTP verified, set user as logged in
      setUser(foundUser);
      localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(foundUser));
