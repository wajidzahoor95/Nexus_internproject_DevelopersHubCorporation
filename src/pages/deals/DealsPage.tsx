...
export const DealsPage: React.FC = () => {
  ...
  const handleFundDeal = (deal: any) => {
    setFundingModal({ isOpen: true, deal });
  };

  const confirmFunding = () => {
    const amount = parseFloat(fundingAmount);
    if (amount > 0 && fundingModal.deal) {
      // Find the entrepreneur for this deal
      const entrepreneur = entrepreneurs.find(
        (e) => e.startupName === fundingModal.deal.startup.name
      );
      if (entrepreneur) {
        fundDeal(
          amount,
          entrepreneur.name,
          fundingModal.deal.id,
          fundingModal.deal.startup.name
        );
        setFundingModal({ isOpen: false, deal: null });
        setFundingAmount("");
      }
    }
  };
